...
import {
  getAllKommuModules,
  getKommuModule,
  getKommuModuleContent,
  getKommuModuleNeighbors,
  TRACK_LABELS,
} from "@/lib/kommu";
...
const T = {
  fr: {
    prev: "Module precedent",
    next: "Module suivant",
    onThisPage: "Sections",
    cta: "S'inscrire a la Beta",
    ctaHref: "/beta",
    track: "Parcours",
    duration: "Duree estimee",
    audience: "Public cible",
    backToHub: "Retour au programme Kommu",
  },
  en: {
    ...
    backToHub: "Back to Kommu program",
  },
} as const;
...
export default async function KommuModuleDetail({
  params,
}: {
  params: Promise<{ locale: string; slug: string }>;
}) {
  ...
  const t = T[key];
  const { prev, next } = getKommuModuleNeighbors(slug);
  const trackLabel = TRACK_LABELS[mod.track]?.[key] ?? mod.track;
    ...
    <PageShell>
      ...
      <div className="border-b border-[color:var(--color-line)]">
        <div className="mx-auto max-w-7xl px-5 py-3 sm:px-6">
          <Link
            href="/kommu"
            className="font-sans text-xs font-semibold uppercase tracking-[0.2em] text-[color:var(--color-muted)] transition-colors duration-150 hover:text-[color:var(--color-ink)]"
          >
            &larr; {t.backToHub}
          </Link>
        </div>
            ...
            <div className="sticky top-24 flex flex-col gap-8 border-t-2 border-[color:var(--color-ink)] pt-8">
              ...
              {mod.duration && (
                <div>
                  ...
                  <p className="mt-2 font-sans text-sm">{mod.duration}</p>
                </div>
              )}
              {mod.audience && (
                <div>
                  ...
                  <p className="mt-2 font-sans text-sm">{mod.audience}</p>
                </div>
              )}
              {toc.length > 0 && (
                <div>
      ...
      <nav className="border-b border-[color:var(--color-line)]">
        <div className="mx-auto flex max-w-7xl items-stretch gap-px bg-[color:var(--color-line)]">
          ...
          >
            ...
            <p className="mt-2 font-serif text-lg font-bold leading-tight">
              {prev.title}
            </p>
          </Link>
          <Link
            href={`/kommu/${next.slug}`}
